...
def timedelta_to_days(td):
    ...
    days = td.days + (td.seconds + (td.microseconds * 1.e-6)) / seconds_in_day
...
class datetime(dt.datetime):
    ...
    """
    #WARNING!!!  Commenting this out may cause issues between different datetime subclasses
    #   I am unsure if there will be consequences or not.  These methods, however, are buggy
    #   and should be modified to not require the timedelta_to_days and datetime_to_jd functions.
    #   There is no reason to perform the math here on the JD days.
