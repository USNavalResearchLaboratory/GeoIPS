...
def check_for_hrit():
    if not XRIT_PATH:
        HritError('No path to xRIT decompression software '
                  'Please set XRIT_DECOMPRESS_PATH environment variable.')
    elif not os.path.isfile(XRIT_PATH):
        HritError('xRITDecompress software not found. '
...
class HritFile(object):
    ...
    def annotation_metadata(self):
        field_names = ['type', 'disseminationID', '_', 'platform',
                       'band', 'segment', 'start_datetime', 'compression']
        fields = {name: part.strip('_') for name, part in
                  zip(field_names, self.metadata['block_4']['annotation'].split('-'))
                  if name != '_'}
        fields['platform'] = fields['platform'].lower()
        fields['start_datetime'] = datetime.strptime(fields['start_datetime'], '%Y%m%d%H%M')
        return fields
        ...
        for block_num in self.block_info.keys():
            ...
            try:
                ...
                if block_map is None:
                    log.debug('Skipping known, but undefined metadata block: {}'.format(block_num))
                    continue
            except KeyError:
                log.warning('Unrecognized metadata block number encountered: {}'.format(block_num))
                continue
            ...
            for field_name, dtype, count in block_map:
                ...
                if dtype.char == 'S':
                    for valnum, val in enumerate(block_md[field_name]):
                        block_md[field_name][valnum] = val.strip()
    ...
    def _read_image_data(self, sector=None):
        ...
        if self.file_type != 'image':
            raise HritError('Unable to read image data from file of type {}'.format(self.file_type))
    ...
    def _read_prologue(self, sector=None):
        ...
        if self.file_type != 'prologue':
            raise HritError('Unable to read prologue data from file of type {}'.format(self.file_type))
        ...
        prologue['radiometricProcessing']['level15ImageCalibration'] = \
        ...
        prologue['radiometricProcessing']['mpefCalFeedback'] = self.__read_impf_cal_data()

        prologue['radiometricProcessing']['radTransform'] = self.__rf('>f4', shape=(42, 64))
        ...
        prologue['geometricProcessing']['atmosphericModel'] = self.__rf('>f4', shape=(12, 360))
        prologue['geometricProcessing']['resamplingFunctions'] = self.__rf('>u1', shape=(12,))
    ...
    def _read_epilogue(self, sector=None):
        ...
        if self.file_type != 'epilogue':
            raise HritError('Unable to read epilogue data from file of type {}'.format(self.file_type))
    ...
    def __read_time_cds(self, expanded=False):
        ...
        if expanded:
            ...
            microsec = 0
            nanosec = 0
        time = epoch + timedelta(days=int(days), milliseconds=int(millisec),
    ...
    def __read_orbit_polynomial(self):
        ...
        for elemind in range(0, 100):
            elem = {'startTime': self.__read_time_cds(),
