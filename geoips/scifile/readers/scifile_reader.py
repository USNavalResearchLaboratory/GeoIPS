...
class SciFile_Reader(Reader):
    ...
    def read(self,fname,datavars,gvars,metadata,chans=None,sector_definition=None):
        ...
        # MLS 20190207 Yup, still a problem. But only sometimes?  Also, sometimes 'shape' is not
        # in the metadata['top'] dictionary - if not, don't try to set.  This should cover all possibilities
        if 'shape' in metadata['top'].keys():
