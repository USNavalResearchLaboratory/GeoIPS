...
class NAVGEMIEEE_BINARY_Reader(Reader):
    ...
    def read(self,fname,datavars,gvars,metadata,chans=None,sector_definition=None):
        ...
        dlat = (90.0  + 90.0)/360.0
        dlon = dlat
        
        navlon1d = np.arange(0.0,360.0,dlon)
        navlat1d = np.arange(-90.0,(90.0+dlat),dlat)
        navlon,navlat = np.meshgrid(navlon1d, navlat1d)
        ...
        def seek_field(filename, metadata_only=False):
            ...
            # bottom to top
            offset = (int(float(1)) - 1) * record_length
...
#            return data, istat
        ...
        for dsname in self.gvar_info.keys():
            for geoipsvarname,dfvarname in self.gvar_info[dsname].items():
                ...
                gvars[dsname][geoipsvarname] = np.ma.array(geolog)
