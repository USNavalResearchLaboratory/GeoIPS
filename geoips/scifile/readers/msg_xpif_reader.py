...
def calculate_latitude_longitude(num_lines, num_samples, line_off, sample_off,
...
def get_geolocation(dt, metadata, sector_definition=None):
    ...
    if sector_definition is not None:
        ...
        if not os.path.isfile(inds_file):
            ...
            log.info('    GETGEOINDS Creating full disk swath definition for '+adname)
    ...
    log.info('GETGEO Must calculate solar zen/azm for sector '+adname)
    sect_sun_zen, sect_sun_azm = calculate_solar_angles(gmd, sect_lats, sect_lons, dt)
    log.info('GETGEO Done calculating solar zen/azm for sector '+adname)
        ...
        for chan, types in chan_info.items():
            ...
            for res, res_chans in self.dataset_info.items():
                if chan in res_chans:
                    break
            if res not in gvars.keys() or not gvars[res]:
                log.interactive('We don\'t have geolocation for {} for {} skipping {}'.format(res, adname, chan))
                continue
        ...
        for ds in datavars.keys():
            if not datavars[ds]:
                ...
                for varname in datavars[ds].keys():
                    datavars[ds][varname] = np.ma.masked_less_equal(datavars[ds][varname], -999.0)
        log.interactive('Done reading SEVIRI data for '+adname)
        log.info('')
