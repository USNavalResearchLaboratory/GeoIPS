...
def extalg(datafile, sector, product, workdir):
    ...
    if write_file:
        if datafile.security_classification:
            dirname = os.getenv('GEOIPS_OUTDIRS') + '/data/preprocessed/{}_{}'.format(
                datafile.source_name, datafile.security_classification.replace('/', '-'))
        else:
            dirname = os.getenv('GEOIPS_OUTDIRS') + '/data/preprocessed/()'.format(datafile.source_name)
        baseh5filename = '{}/{}_{}'.format(
            dirname, sector.name, '_'.join(sorted(datafile.datasets.keys())))
