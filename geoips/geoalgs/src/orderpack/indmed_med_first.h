integer(kind=kk), dimension (:), intent (in) :: idatt
integer(kind=kk), intent (out) :: ires_med

integer(kind=kk), dimension (7*(((size (idatt)+6)/7+6)/7)) :: istrt, iendt, imedt
integer(kind=kk), dimension (7*((size(idatt)+6)/7)) :: iwrkt
integer(kind=kk) :: ntri, nmed, nord, nequ, nleq, imed, idon, idon1
integer(kind=kk) :: ideb, itmp, idcr, icrs, icrs1, icrs2, imax, imin
integer(kind=kk) :: iwrk, iwrk1, imed1, imed7, ndat

!
! This is ripped from orderpack
! This is only the first half of the "med" routine
! The second half is in med_second.h
!
      NDAT = Size (IDATT)
      NMED = (NDAT+1) / 2
      IWRKT = IDATT
